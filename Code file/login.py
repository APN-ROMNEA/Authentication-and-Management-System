...
import time
import json
from encryptions import encrypt_data, load_or_generate_key, decrypt_data


COOLDOWN_TIME_USERNAME = 6
COOLDOWN_TIME_LOGIN = 6
COOLDOWN_TIME_CODE = 3
MAX_ATTEMPTS = 3
MAX_ERROR = 5
COOLDOWN_ERROR = 20

class Login:
    def __init__(self, database=r"C:\Users\User\OneDrive\CADT SChool\CADT_Year-2\Python\Week-10\G2_T3_Project\Main\database.txt"):
        ...
        try:
            with open(self.database, "r") as file:
                try:
                    ...
                    print("Error reading the user data file. It may be corrupted.")
                    return False
        except Exception as e:
            print(f"Error opening user data file: {e}")
            return False

        # Check if the username exists
        key = load_or_generate_key()
        normal_text = decrypt_data(key)
        for user_data in users:
            if user_data.get("username") == username:
                checked_password = normal_text.decrypt(user_data.get("password").encode()).decode()
